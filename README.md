...
## BLE протокол
- Основной API: готовые программы слайдера (`Ping-Pong` по умолчанию), их параметры и
  `START/STOP` — [формат и рекомендации для пульта](docs/11_program_api.md)
...
`F/B`, Position, Current и legacy Speed относятся к Advanced API. Движение в сторону уже
нажатого концевика блокируется прошивкой независимо от источника команды.

Подробности Advanced API и старого формата: `docs/03_ble_protocol.md`.
